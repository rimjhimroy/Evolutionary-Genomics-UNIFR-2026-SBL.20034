...
1. ddRADseq dataset: this dataset was produced by reducing genome complexity through double-digest RAD sequencing on 64 individuals and yielded 1,127 SNPs after quality control and pruning of SNPs in linkage disequilibrium, using a pairwise LD threshold such as r² > 0.2.
2. Whole-genome sequencing dataset: this dataset is based on resequencing of a subset of 48 individuals and retains 11,475 SNPs after similar filtering for quality and LD (r² > 0.2).
...
- `dataset-tutorial3/`: the input files organised into numbered datasets
...
```{r, eval=FALSE}
system("mkdir -p /home/rstudio/work/outputs")
```	
...
```{r, eval=FALSE}
input_dir <- "/home/rstudio/work/dataset-tutorial3/01-structure-ddrad/01-structure-input"
setwd(input_dir)
getwd()
list.files()
```
...
```{r, eval=FALSE}
dir.create("/home/rstudio/work/outputs/01-structure-ddrad/small_structure_example", recursive = TRUE, showWarnings = FALSE)
setwd("/home/rstudio/work/outputs/01-structure-ddrad/small_structure_example")
getwd()
```
...
```{r, eval=FALSE}
small_run_dir <- "/home/rstudio/work/outputs/01-structure-ddrad/small_structure_example"
input_file <- "/home/rstudio/work/dataset-tutorial3/01-structure-ddrad/01-structure-input/diploids_filt1rm_biSNPs_minDP15maxDP130nc_MAF005rm_MD02_pruned02_structure"

file.copy(
	from = input_file,
	to = file.path(small_run_dir, basename(input_file)),
	overwrite = TRUE
)

list.files(small_run_dir)
```
...
writeLines(
	...
	con = file.path(small_run_dir, "mainparams")
)
```
...
writeLines(
	c(
		...
		"#define USEPOPINFO 0        # Unsupervised clustering",
	...
	con = file.path(small_run_dir, "extraparams")
)
```

These parameters configure STRUCTURE to perform **unsupervised population clustering with ancestry coefficients and model likelihood and statistical outputs**. Setting `NOADMIX 0` activates the admixture model, which allows each individual to draw a proportion of their ancestry from multiple clusters. `LINKAGE 0` tells STRUCTURE to treat all loci as statistically independent, which is the correct assumption here because our input SNPs have already been LD-pruned. `USEPOPINFO 0` and `LOCISPOP 0` tell STRUCTURE to discover population structure *de novo* without being guided by metadata; the algorithm finds clusters purely from genetic similarity. `INFERALPHA 1` allows STRUCTURE to learn the Dirichlet parameter alpha from the data itself, while `FREQSCORR 1` models realistic genetic differentiation between clusters by allowing allele frequencies to vary systematically across populations. `COMPUTEPROB 1` calculates the likelihood P(data|K), StructureHarvester later uses these values across replicate runs and K values to calculate Evanno’s ΔK statistic, `PRINTNET 1` outputs pairwise genetic distances between clusters, and `PRINTQHAT 1` outputs ancestry proportions for each individual. Finally, `UPDATEFREQ 5000` controls how frequently STRUCTURE prints progress updates during the run (every 5000 steps), and `RANDOMIZE 0` ensures parameters are updated in deterministic order for reproducible results across runs, promoting convergence stability.  

Make a folder for the test outputs:

```{r, eval=FALSE}
dir.create("/home/rstudio/work/outputs/01-structure-ddrad/small_structure_example/small_structure_runs", showWarnings = FALSE)
```
...
small_runs_dir <- "small_structure_runs"
small_logs_dir <- file.path(small_runs_dir, "logs")

jobs <- expand.grid(K = 1:4, REP = 1:3)

dir.create(small_logs_dir, showWarnings = FALSE)

parallel::mclapply(
	seq_len(nrow(jobs)),
	function(i) {
		output_prefix <- file.path(small_runs_dir, sprintf("run_K%d_rep%d", jobs$K[i], jobs$REP[i]))
		log_file <- file.path(small_logs_dir, sprintf("run_K%d_rep%d.log", jobs$K[i], jobs$REP[i]))

		system(
			sprintf(
				"structure -K %d -m %s -e %s -o %s > %s 2>&1",
				jobs$K[i],
				mainparams_file,
				extraparams_file,
...
```{r, eval=FALSE}
system("ls /home/rstudio/work/outputs/01-structure-ddrad/small_structure_example/small_structure_runs")
```

You should see files ending in `_f` and `_q`.  
The `_f` file is the full STRUCTURE output for one run, including run settings, likelihood values, and the detailed ancestry report for each individual. The `_f` file also reports summary statistics that help describe the clustering result in more detail. These include values that show how different the inferred clusters are from one another, how much genetic variation is found within each cluster, and how strongly each cluster is genetically differentiated. You can open one `_f` file and look for the corresponding section headings, such as  `Overall proportion of membership`, `Estimated Ln Prob of Data`, `Inferred ancestry of individuals`, `Allele-freq. divergence among pops`, `Average distances`, and `Mean value of Fst`.  
The `_q` file is the corresponding ancestry-coefficient table, with one line per individual and the estimated proportion of membership in each inferred cluster or the Q values.  
...
```{r, eval=FALSE}
setwd("/home/rstudio/work/dataset-tutorial3/01-structure-ddrad/02-precomputed-structure-runs")
list.files()
setwd("/home/rstudio/work/dataset-tutorial3/01-structure-ddrad/02-precomputed-structure-runs/ddRAD_results")
list.files()
```
...
```{r, eval=FALSE, message=FALSE}
dir.create("/home/rstudio/work/outputs/01-structure-ddrad", recursive = TRUE, showWarnings = FALSE)
system(
	paste(
...
```{r, eval=FALSE}
system("ls /home/rstudio/work/outputs/01-structure-ddrad/structureHarvester_analysis/*.indfile")
```
...
```{r, eval=FALSE, message=FALSE}
clumpp_dir <- "/home/rstudio/work/outputs/01-structure-ddrad/structureHarvester_analysis"

file.copy(
	from = "/home/rstudio/work/dataset-tutorial3/01-structure-ddrad/03-reference-clumpp-files/paramfile",
	to = file.path(clumpp_dir, "paramfile"),
	overwrite = TRUE
)
```
...
for (K in 2:6) {
	...
	system(
		sprintf(
			"CLUMPP -k %d -r %d -c %d -i %s -o %s -j %s",
			K,
			REPS,
			INDS,
			sprintf("K%d.indfile", K),
			OUTFILE,
			sprintf("K%d.miscfile", K)
		), intern = TRUE
	)
}
```
...
Now open the plotting script in RStudio:

`Evolutionary-Genomics-UNIFR-2026-SBL.20034/practical3/scripts/01-tutorial-structure-ddrad.Rmd`

### What the plotting script does

The script reads the summary files you already prepared with StructureHarvester and CLUMPP, then turns them into figures.

It will:

- read the likelihood summary for K = 1 to K = 6 and inspect Evanno’s ΔK for K values where it is defined
- decide which value of `K` is best supported. **PLEASE CHOOSE THIS VALUE CAREFULLY as 5 is used here as an example. You will need to overwrite your chosen value of `K` into the `best_k` chunk of the notebook.**
- read the aligned CLUMPP membership coefficients for the `K` you chose
- plot the population locations and show the mean admixture proportions for each population
...
1. Run the model-choice chunks first and inspect `structure_model_choice.png`.
2. In the `choose-best-k` chunk, fill in the value of `best_k` that you infer from the model-choice figure. Choose the best K based on the Evanno method (Evanno et al 2005), where delta K reaches its highest peak.
3. Run the remaining chunks to draw the ancestry barplot and population map for your chosen `K`.
...
- `structure_k5_membership.tsv` if you set `best_k <- 5`
- `structure_k5_barplot.png` if you set `best_k <- 5`
...
- `structure_k5_barplot.png`: each bar is one individual; coloured segments represent estimated ancestry proportions assigned to each inferred cluster when `best_k <- 5`
...
4. Explain the pattern observed in the North of the Alps (e.g. field population V2B)?
...
`dataset-tutorial3/02-ddrad-wgs-population-comparison/`

Script:

`Evolutionary-Genomics-UNIFR-2026-SBL.20034/practical3/scripts/02-tutorial-ddrad-wgs-comparison.Rmd`

### What this script does

This script asks whether ddRAD and WGS tell the same broad biological story. Differences between ddRAD and WGS may reflect marker density, missing data, or genomic distribution of markers, not only biological differences.
...
5. Do ddRAD and WGS agree on which population pairs are most differentiated?

## Part 3 - Sliding-Window Differentiation Along Scaffold 1

Dataset folder:

`dataset-tutorial3/03-scaffold1-window-scan/`

Script:

`Evolutionary-Genomics-UNIFR-2026-SBL.20034/practical3/scripts/03-tutorial-scaffold1-window-scan.Rmd`
...
2. Does WGS reveal a clearer pattern than ddRAD?
