...
```r
setwd("dataset-tutorial3/01-structure-ddrad/01-structure-input")
getwd()
list.files()
```
...
```r
system("mkdir -p small_structure_runs")
```
...
```r
setwd("../02-precomputed-structure-runs")
list.files()
setwd("ddRAD_results")
list.files()
```
...
```r
setwd("..")
system("structureHarvester.py --dir ddRAD_results --out structureHarvester_analysis --evanno --clumpp")
```
...
```r
system("ls structureHarvester_analysis")
```
...
```r
setwd("structureHarvester_analysis")
system("ls *.indfile")
```
...
4. In the `choose-best-k` chunk, fill in the value of `best_k` that you infer from the model-choice figure.
...
4. Which value did you choose for `best_k`, and does the ancestry plot for that `K` support the same interpretation?
