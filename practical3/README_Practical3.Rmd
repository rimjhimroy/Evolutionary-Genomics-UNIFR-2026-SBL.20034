...
```{r, eval=FALSE}
small_run_dir <- "/home/rstudio/work/outputs/01-structure-ddrad/small_structure_example"
mainparams_file <- file.path(small_run_dir, "mainparams")
extraparams_file <- file.path(small_run_dir, "extraparams")
small_runs_dir <- file.path(small_run_dir, "small_structure_runs")
...
for (K in 2:6) {
	OUTFILE <- file.path(clumpp_dir, sprintf("clumpp_output_%d.txt", K))
	system(
		sprintf(
			...
			REPS,
			INDS,
			shQuote(file.path(clumpp_dir, sprintf("K%d.indfile", K))),
			shQuote(OUTFILE),
			shQuote(file.path(clumpp_dir, sprintf("K%d.miscfile", K)))
		)
	)
}
```
