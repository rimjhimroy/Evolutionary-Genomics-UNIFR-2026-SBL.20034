...
parallel::mclapply(
	...
	function(i) {
		system(
			sprintf(
				"structure -K %d -m %s -e %s -o %s",
				jobs$K[i],
				shQuote(mainparams_file),
				shQuote(extraparams_file),
				shQuote(file.path(small_runs_dir, sprintf("run_K%d_rep%d", jobs$K[i], jobs$REP[i])))
			), intern = TRUE
		)
	},
	mc.cores = 4
...
- `structure_k8_membership.tsv` if you set `best_k <- 8`
- `structure_k8_barplot.png` if you set `best_k <- 8`
...
- `structure_k8_barplot.png`: each bar is one individual; the colored fractions show the estimated ancestry membership in each cluster when `best_k <- 8`
